...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 16, 2025 at 07:17 AM
...
UPDATE `login` SET `id_login` = 1,`username` = 'admin',`password` = 'admin123' WHERE `login`.`id_login` = 1;
UPDATE `login` SET `id_login` = 2,`username` = 'admin',`password` = 'admin' WHERE `login`.`id_login` = 2;
...
UPDATE `rak` SET `id_rak` = 'A1',`keterangan` = NULL WHERE `rak`.`id_rak` = 'A1';
...
ALTER TABLE `penelitian`
ADD FULLTEXT(`judul`, `nama_penulis`);
COMMIT;
